...
async function initDB() {
    ...
    if (!hasSettings) {
        ...
        await run(`INSERT INTO settings (key, value) VALUES ('delivery_time', '40 minutos')`); // Prazo estimado
    } else {
        // Atualizações para bases antigas que não têm os novos campos (MANTÉM DADOS EXISTENTES)
        ...
        const hasDelTime = await get(`SELECT value FROM settings WHERE key = 'delivery_time'`);
        if(!hasDelTime) await run(`INSERT INTO settings (key, value) VALUES ('delivery_time', '40 minutos')`);
    }
...
app.post('/api/orders', async (req, res) => {
    ...
    try {
        ...
        triggerWebhook({
            event: 'new_order',
            orderId: result.lastID, whatsapp, name, address, type, paymentMethod, items, subtotal, delivery_fee, total, status: 'Pendente',
...
// [NOVO] Excluir item do cardápio permanentemente
app.delete('/api/admin/items/:id', authAdmin, async (req, res) => {
    try {
...
app.put('/api/admin/settings', authAdmin, async (req, res) => {
    const { delivery_fee, webhook_url, is_open, unavailable_flavors, delivery_time } = req.body;
    try {
        ...
        await run(`INSERT INTO settings (key, value) VALUES ('delivery_time', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value`, [delivery_time]);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
